...
# CSS personalizado con fondo azul claro
def load_css():
    st.markdown("""
    <style>
...
def init_database():
    ...
    # Insertar doctores de ejemplo (sin duplicados)
    cursor.execute('DELETE FROM doctores')  # Limpiar duplicados
    doctores_default = [
    ...
    for doctor in doctores_default:
        cursor.execute('INSERT INTO doctores (nombre, clinica, especialidad, telefono, email, categoria, descuento) VALUES (?, ?, ?, ?, ?, ?, ?)', doctor)
    ...
    # Crear usuarios doctores (sin duplicados)
    doctores_usuarios = [
    ...
    notificaciones_default = [
        (1, 'Nueva Orden Creada', 'Se ha creado la orden ORD-001 para el Dr. Juan Guillermo', 'info'),
        (1, 'Stock Crítico', 'El inventario de Zirconio Blocks está en nivel crítico', 'warning'),
        (6, 'Orden Lista para Entrega', 'La orden ORD-001 está lista para ser entregada', 'success')
    ]
    
    for notif in notificaciones_default:
        cursor.execute('INSERT OR IGNORE INTO notificaciones (usuario_id, titulo, mensaje, tipo) VALUES (?, ?, ?, ?)', notif)
...
def show_navigation():
    ...
    <div class="navigation-menu">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <div>
                <span style="color: white;">🔔 Notificaciones ({notification_count})</span>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if notification_count > 0:
        with st.expander(f"📬 Ver {notification_count} Notificaciones"):
            for _, notif in notifications.iterrows():
                st.markdown(f"""
                <div class="notification-info">
                    <h4 style="margin: 0; color: #333;">{notif['titulo']}</h4>
                    <p style="margin: 5px 0; color: #666;">{notif['mensaje']}</p>
                    <small style="color: #999;">📅 {notif['fecha_creacion']}</small>
                </div>
                """, unsafe_allow_html=True)
...
def modulo_ordenes():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">📋 Gestión de Órdenes</h1>
    """, unsafe_allow_html=True)
...
# Módulo de doctores con edición (SIN DUPLICADOS)
def modulo_doctores():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">👨‍⚕️ Gestión de Doctores</h1>
    """, unsafe_allow_html=True)
    ...
    if not doctores_df.empty:
        ...
        doctores_df = doctores_df.drop_duplicates(subset=['nombre'], keep='first')
...
def modulo_inventario():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">📦 Control de Inventario</h1>
    """, unsafe_allow_html=True)
...
def modulo_reportes():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">📊 Reportes y Análisis</h1>
    """, unsafe_allow_html=True)
...
def modulo_usuarios():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">👥 Gestión de Usuarios</h1>
    """, unsafe_allow_html=True)
...
# Módulo de seguimiento COMPLETO
def modulo_seguimiento():
    st.markdown("""
    <div class="content-container">
        <h1 style="color: white; text-align: center; margin-bottom: 20px;">📍 Seguimiento de Envíos</h1>
    """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            if st.button("🚀 Crear Orden", type="primary", use_container_width=True):
                if paciente and tipo_trabajo:
                    ...
                    cursor.execute('''
                        INSERT INTO ordenes (numero_orden, doctor_id, paciente, tipo_trabajo, descripcion, 
                                           fecha_entrega_estimada, precio, observaciones, estado, tracking_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (numero_orden, doctor['id'], paciente, tipo_trabajo, descripcion, 
                          fecha_entrega_deseada, precio_final, observaciones, 'Creada', tracking_id))
...
def main():
    ...
    # Mostrar el módulo correspondiente según el rol
    if st.session_state.user['rol'] == 'Doctor':
        # Los doctores SOLO ven su portal, NO el dashboard
        portal_doctores()
    else:
        # Personal del laboratorio ve los módulos según su rol
        if current_module == 'Dashboard':
            ...
            modulo_doctores()
        elif current_module == 'Inventario':
            modulo_inventario()
        elif current_module == 'Reportes':
            modulo_reportes()
